...
import { Eye, EyeSlash, CheckCircle, XCircle } from 'phosphor-react'
                ...
                return (
                  <Form noValidate className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      <Label htmlFor="fullName" className="text-[14px] font-semibold text-[#868686]">Full name</Label>
                      <Input id="fullName" name="fullName" placeholder="Juan Dela Cruz" value={values.fullName} onChange={handleChange} onBlur={handleBlur} className="w-full h-[42px] px-[16px] rounded-[6px] border border-[#d9d9d9] !text-[14px] placeholder:text-[14px] placeholder:text-[#d9d9d9] font-normal" />
                    </div>
                    {/* Phone Number */}
                    <div className="flex flex-col gap-1 w-full">
                      <Label htmlFor="phone" className="text-[14px] font-semibold text-[#868686]">Phone number</Label>
                      <div className="flex w-full h-[42px] bg-white rounded-[6px] border border-[#d9d9d9] items-center">
                        <span className="flex items-center px-2 text-[#868686] text-[16px] relative">+639</span>
                        <Input id="phone" name="phone" placeholder="00 0000 000" value={values.phone} onChange={handleChange} onBlur={handleBlur} className="flex-1 h-full px-[16px] rounded-r-[6px] border-none !text-[14px] placeholder:text-[14px] placeholder:text-[#d9d9d9] font-normal" />
                      </div>
                    </div>
                    {/* Email Address */}
                    <div className="flex flex-col gap-1 w-full">
                      <Label htmlFor="email" className="text-[14px] font-semibold text-[#868686]">Email</Label>
                      <Input id="email" name="email" type="email" placeholder="name@email.com" value={values.email} onChange={handleChange} onBlur={handleBlur} className="w-full h-[42px] px-[16px] rounded-[6px] border border-[#d9d9d9] !text-[14px] placeholder:text-[14px] placeholder:text-[#d9d9d9] font-normal" />
                    </div>
                    {/* Password */}
                    <div className="flex flex-col gap-1 w-full">
                      <Label htmlFor="password" className="text-[14px] font-semibold text-[#868686]">Enter password</Label>
                      <div className="relative w-full">
                        <Input id="password" name="password" type={showPassword ? 'text' : 'password'} placeholder="Enter password" value={values.password} onChange={handleChange} onBlur={handleBlur} className="w-full h-[42px] px-[16px] pr-10 rounded-[6px] border border-[#d9d9d9] !text-[14px] placeholder:text-[14px] placeholder:text-[#d9d9d9] font-normal" />
                        <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2" tabIndex={-1} onClick={() => setShowPassword(v => !v)}>
                          {showPassword ? (
                      ...
                      <div className="flex gap-1 mt-2 h-1">
                        {(() => {
                          const checks = [/[A-Z]/.test(values.password), /[0-9]/.test(values.password), values.password.length >= 8];
                          const strength = checks.filter(Boolean).length;
                          let colors = ['bg-[#d9d9d9]', 'bg-[#d9d9d9]', 'bg-[#d9d9d9]'];
                          if (strength === 1) {
                            colors = ['bg-[#ffe066]', 'bg-[#d9d9d9]', 'bg-[#d9d9d9]']; // yellow
                          } else if (strength === 2) {
                            colors = ['bg-[#ffa600]', 'bg-[#ffa600]', 'bg-[#d9d9d9]']; // orange
                          } else if (strength === 3) {
                            colors = ['bg-[#28c700]', 'bg-[#28c700]', 'bg-[#28c700]']; // green
                          }
                          return colors.map((c, i) => <span key={i} className={`flex-1 rounded-[999px] ${c}`}></span>);
                        })()}
                      </div>
                      ...
                      <div className="mt-2 flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-[14px] font-normal">
                          {/[A-Z]/.test(values.password)
                            ? <CheckCircle size={20} color="#28c700" weight="fill" />
                            : <XCircle size={20} color="#f25d3b" weight="fill" />}
                          <span className={`${/[A-Z]/.test(values.password) ? 'text-[#363636]' : 'text-[#868686]'}`}>At least one capital letter</span>
                        </div>
                        <div className="flex items-center gap-2 text-[14px] font-normal">
                          {/[0-9]/.test(values.password)
                            ? <CheckCircle size={20} color="#28c700" weight="fill" />
                            : <XCircle size={20} color="#f25d3b" weight="fill" />}
                          <span className={`${/[0-9]/.test(values.password) ? 'text-[#363636]' : 'text-[#868686]'}`}>At least one number</span>
                        </div>
                        <div className="flex items-center gap-2 text-[14px] font-normal">
                          {values.password.length >= 8
                            ? <CheckCircle size={20} color="#28c700" weight="fill" />
                            : <XCircle size={20} color="#f25d3b" weight="fill" />}
                          <span className={`${values.password.length >= 8 ? 'text-[#363636]' : 'text-[#868686]'}`}>Be at least 8 characters</span>
                        </div>
                      </div>
                    </div>
                    {/* Terms and Conditions */}
                    <div className="flex items-start gap-2 mt-4 w-full">
                      <input type="checkbox" id="terms" name="terms" checked={values.terms} onChange={e => setFieldValue('terms', e.target.checked)} className="accent-[#383ad8] w-4 h-4 rounded-[4px] border border-[#d9d9d9]" />
                      <label htmlFor="terms" className="text-[14px] text-[#363636] font-normal leading-[1.5]">
                        I acknowledge that I have read and accept the
                        <a href="#" className="text-[#383ad8] font-semibold ml-1">Terms and Condition</a>
                        <span> and the </span>
                        <a href="#" className="text-[#383ad8] font-semibold">Privacy Policy</a>.
                      </label>
                    </div>
                    {/* Sign Up Button */}
                    <button type="submit" className="w-full h-[42px] bg-[#383ad8] text-[white] text-[14px] font-semibold rounded-[6px]" disabled={isLoading || !values.terms}>
